...
// I was about to write a new routine for generating transversals in general by doing a
// BFS of the Cayley graph.  This, again, would have no clear stopping condition.
// But!  I found something called the orbit algorithm that will find transversals
// for the specific kind of subgroup I care about, which is the stabilizer subgroup.
// http://www.math.colostate.edu/~hulpke/talks/decomp.pdf
// It is easy to show that if we find a set of permutations that take a point to
// all of its orbits, then that set of permutations...(wait for it...) is the transversal!!!!

// This algorithm is intended to generate all elements of the group that is generated by
// the given generator set.  Unfortunately, as of this writing, it has no reliable stopping
// condition, but it appears to be a good approach to the problem.  It should be noted that
// the problem is trivial in the case of Abelian groups, but we support no such assumtion here.
// This routine can be used to generate a transversal in the case that we're asked to generate
// a factor group, which, of course, requires a normal subgroup.  If the subgroup is not normal,
// however, I cannot conclude whether the result will of this method will be a full or partial
// set of the desired transversal.  Transversals of a non-normal subgroup need to be considered
// in and generated by a seperate method.  The need for more research is clearly indicated.
