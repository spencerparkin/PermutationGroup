...
// TODO: What might be a better algorithm here is to do a BFS on the Cayley graph of the group,
//       starting with the identity.  Generators take you along adjacencies.  If the generating
//       set is irreducible, it's possible that the algorithm can terminate when its queue becomes
//       empty.  A frontier is pushed into the graph only when doing so would take us to an element
//       that we have not yet encountered.  The correctness of such an algorithm is not immediately
//       obvious to me; it might not be correct.
